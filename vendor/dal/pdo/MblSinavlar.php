 ...
 * @author Okan CIRAN
 */
class MblSinavlar extends \DAL\DalSlim {
    ...
    public function getSinavTurleri($params = array()) {
        try {
            ...
            if (isset($params['KurumGrupID']) && $params['KurumGrupID'] != "") {
                $SinavTuruId = $params['KurumGrupID'];
            } 
            $sql = "  
            SET NOCOUNT ON;  
            SELECT * FROM (           
            ...
            UNION
                SELECT    
                    av.SinavTurID as id ,
                    upper( COALESCE(NULLIF(avx.SinavTurAdi,''),avx.SinavTurAdiEng)) as aciklama,  
                    upper(avx.SinavTurAdiEng) as description_eng
                FROM [BILSANET_TAKEVBODRUM].[dbo].[SNV_SinavTurleri] av
                LEFT JOIN BILSANET_MOBILE.dbo.sys_language lx ON lx.id =".$languageIdValue." AND lx.deleted =0 AND lx.active =0
                LEFT JOIN BILSANET_MOBILE.dbo.Mobil_SNVSinavTurleri_lng  avx on avx.SinavTurID = av.sinavTurID and  avx.language_id= lx.id
                WHERE av.KurumGrupID = ".$SinavTuruId."  
             ) as  ssss
             ORDER BY ssss.id
            SET NOCOUNT OFF;  
                "; 
