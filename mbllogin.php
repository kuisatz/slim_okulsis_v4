    ...
    foreach ($resDataMenu as $menu){
        $menus[]  = array(
           ...
           /* "ParentID" => $menu["ParentID"],*/
            ...
            "alan11" => html_entity_decode($menu["alan11"]),
            "alan12" => html_entity_decode($menu["alan12"]),
            "alan13" => html_entity_decode($menu["alan13"]),
            "alan14" => html_entity_decode($menu["alan14"]),
            "alan15" => html_entity_decode($menu["alan15"]),
            "alan16" => html_entity_decode($menu["alan16"]),
            "alan17" => html_entity_decode($menu["alan17"]),
 ...
 * @since 03.10.2017
 */
$app->get("/KyOgretmeninOdevListesiSinifBazli_mbllogin/", function () use ($app ) {
    $stripper = $app->getServiceManager()->get('filterChainerCustom');
    $stripChainerFactory = new \Services\Filter\Helper\FilterChainerFactory();
    $BLL = $app->getBLLManager()->get('mblLoginBLL'); 
 ...
 * @since 03.10.2017
 */
$app->get("/KyOgretmeninOdevListesiOgrenciBazli_mbllogin/", function () use ($app ) {
    $stripper = $app->getServiceManager()->get('filterChainerCustom');
    $stripChainerFactory = new \Services\Filter\Helper\FilterChainerFactory();
    $BLL = $app->getBLLManager()->get('mblLoginBLL'); 
 ...
 * @since 25-10-2017 
 */
$app->get("/SinavTurleriCombo_mbllogin/", function () use ($app ) {
    $stripper = $app->getServiceManager()->get('filterChainerCustom');
    $stripChainerFactory = new \Services\Filter\Helper\FilterChainerFactory();        
    $BLL = $app->getBLLManager()->get('mblLoginBLL'); 
    $headerParams = $app->request()->headers(); 
    ...
    foreach ($resDataMenu as $menu){
        $menus[]  = array(
            "SinavTurID" => $menu["SinavTurID"], 
            "SinavTurAdi" => html_entity_decode($menu["SinavTurAdi"]),   
            "SinavTurAciklama" => html_entity_decode($menu["SinavTurAciklama"]),   
